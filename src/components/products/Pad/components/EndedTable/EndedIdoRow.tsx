...
export default function EndedIdoRow({ ido }: { ido: any }) {
    ...
    return (
        <div className="border-b last:border-0 border-[#112B40] flex py-6 cursor-pointer" onClick={handleClickProject}>
            ...
            <div className="min-w-[80px] justify-end text-[16px] flex items-center" style={{ width: "9%" }}>{userDepositedAmount ? `${userDepositedAmount.toLocaleString()} ${userDepositToken?userDepositToken.symbol:''} ` : ''}</div>
            ...
            <div className="min-w-[100px] justify-end text-[16px] flex items-center" style={{ width: "10%" }}>{`${ido?.allTimeHigh?'$'+ido?.allTimeHigh:''}`}</div>
